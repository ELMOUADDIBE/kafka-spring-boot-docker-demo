...
public class PageEventController {
    ...
    public Flux<Map<String, Double>> analytics() {
        return Flux.interval(Duration.ofSeconds(1))
                .map(seq -> {
                    ...
                    KeyValueIterator<Windowed<String>, Double> windowedIterator = stats.fetchAll(from, now);
                    while (windowedIterator.hasNext()) {
                        KeyValue<Windowed<String>, Double> next = windowedIterator.next();
    ...
    public Flux<Map<String,Long>> analyticsWindows(){
        return Flux.interval(Duration.ofSeconds(1))
                .map(seq->{
                    ...
                    ReadOnlyWindowStore<String, Long> stats = interactiveQueryService.getQueryableStore("total-store", QueryableStoreTypes.windowStore());
                    Instant now=Instant.now();
                    Instant from=now.minusSeconds(30);
